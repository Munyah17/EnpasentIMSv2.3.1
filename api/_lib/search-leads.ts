...
async function extractLeadsWithAI(apiKey: string, results: GoogleSearchResult[], query: string): Promise<Array<{
  ...
  const prompt = `You are helping Enpasent Multiple Agent, a Zimbabwean insurance broker, find potential customers from web search results. The search was for: "${query}".
