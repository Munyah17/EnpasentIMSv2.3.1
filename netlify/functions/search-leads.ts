...
async function extractLeadsWithAI(apiKey: string, results: GoogleSearchResult[], query: string): Promise<Array<{
  ...
  const prompt = `You are helping a Zimbabwean micro-insurance agency (Enpassent Multiple Agent) find potential customers from web search results. The search was for: "${query}".
